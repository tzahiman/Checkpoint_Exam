...
#### SSM Module
- SSM Parameter Store for API auth value (generated by Terraform `random_password` and passed to SSM)
...
- Auth validation from SSM Parameter Store (value generated by Terraform, stored in SSM)
...
### Secrets Management
- API auth value generated by Terraform (`random_password`) and stored in SSM Parameter Store (SecureString)
- Encrypted at rest; no secrets in source code
- Accessed via IAM roles; retrieve once with `terraform output -raw api_auth_value`
